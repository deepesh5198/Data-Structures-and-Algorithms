...
root2.left.left.left = Tree(60)
root2.left.left.right = Tree(70)
root2.left.left.left.left = Tree(88)

def structurallySame(root1, root2):
    if ( (not root1.left) and (not root1.right) )\
        and ( (not root2.left) and (not root2.right) )\
        and (root1.data == root2.data):

        return True 

    elif (root1.data != root2.data)\
        or (root1.left and not root2.left)\
        or (root1.right and not root2.right)\
        or (not root1.left and root2.left)\

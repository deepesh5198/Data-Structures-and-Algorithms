...
# Problem4. print list from the end
...
class SLinkedList:
    ...
    def is_length_even(self):
        # answers is length even?
        current = self.head             #make current node HEAD
        while current != None and current.next != None:     #check if current is None and current_next is None
            current = current.next.next                     #check make current current_next's next
        if current == None:                                 #check current is None if True EVEN
            print(True)
        else:                                               #else FALSE
            print(False)

#problem 11: get all the elements that are smaller than the value K from Heap
...
def getValuesSmallerThan(maxheap, K):
    if K < maxheap.findMin():
        return 
    else:
        for i in range(1, maxheap.size+1):
            if maxheap.heap[i] < K:
                print(maxheap.heap[i])


maxHeap.print_heap()

getValuesSmallerThan(maxHeap, 84)
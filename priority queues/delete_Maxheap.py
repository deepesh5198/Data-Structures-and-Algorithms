...
#function to delete element at index in MaxHeap
def delete_at(maxheap, indx):
    if indx >= maxheap.size:
        print("index out of range")
        return
    else:
        key = maxheap.heap[indx]
        maxheap.heap[indx] = maxheap.heap[maxheap.size]
        maxheap.size -= 1
        maxheap.maxHeapify(indx)
        return key


print(delete_at(maxHeap, indx= 3))
maxHeap.print_heap()